...
export function createDefinition({ key, version = 1, name, states, transitions, status = 'published' }) {
  const r = db.prepare(
    `INSERT INTO workflow_definitions (key, version, name, states, transitions, status)
     VALUES (?, ?, ?, ?, ?, ?)`
...
export function getPublishedDefinition(key) {
  return db.prepare("SELECT * FROM workflow_definitions WHERE key = ? AND status = 'published' ORDER BY version DESC LIMIT 1").get(key);
}

export function listDefinitions() {
  return db.prepare('SELECT * FROM workflow_definitions ORDER BY key, version DESC').all();
}

export function startInstance(definitionKey, entityType, entityId, context = {}, initialState = null) {
  const def = getPublishedDefinition(definitionKey);
  ...
  try {
    ...
    logHistory(instanceId, null, startState, null, null, 'started');
    return { instanceId, currentState: startState };
  } catch (e) {
    if (String(e).includes('UNIQUE constraint failed')) {
      const existing = db.prepare('SELECT * FROM workflow_instances WHERE definition_id = ? AND entity_type = ? AND entity_id = ?').get(def.id, entityType, String(entityId));
...
export function listInstances({ definitionKey, state, entityType, limit = 100 } = {}) {
  let sql = `SELECT wi.*, wd.key AS def_key, wd.name AS def_name FROM workflow_instances wi
             JOIN workflow_definitions wd ON wd.id = wi.definition_id WHERE 1=1`;
  const params = [];
  if (definitionKey) { sql += ' AND wd.key = ?'; params.push(definitionKey); }
...
function logHistory(instanceId, fromState, toState, actorId, actorRole, reason) {
  db.prepare(
...
export function transition(instanceId, targetState, { actorId = null, actorRole = null, reason = null } = {}) {
  ...
  logHistory(instanceId, inst.current_state, targetState, actorId, actorRole, reason);

  publish(
    `WorkflowTransitioned`,
    { instanceId, definitionKey: def.key, fromState: inst.current_state, toState: targetState, reason },
    'workflow',
  ...
  const canonical = canonicalMap[def.key]?.[targetState];
  if (canonical) {
    publish(canonical, { instanceId, state: targetState }, 'workflow', { userId: actorId, entityType: inst.entity_type, entityId: inst.entity_id });
  }
...
export function seedDefaultWorkflows() {
  ...
  for (const d of defs) {
    const exists = db.prepare('SELECT 1 FROM workflow_definitions WHERE key = ? AND version = 1').get(d.key);
    if (!exists) createDefinition({ ...d, version: 1 });
  }
}
