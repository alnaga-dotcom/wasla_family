import { db } from './db.js';

export async function updateUserTrustLevel(userId) {
  const level = await trustLevel(userId);
  await db.prepare('UPDATE users SET trust_level = ? WHERE id = ?').run(level, userId);
  return level;
}

async function fields(userId) {
  const map = {};
  const rows = await db.prepare('SELECT field_key, value FROM profile_fields WHERE user_id = ?').all(userId);
  rows.forEach((r) => { map[r.field_key] = r.value; });
  return map;
}

// مستويات التوثيق — Wasla_14/16:
// L1: رقم موثّق عبر OTP (تسجيل فعّال)
// L2: تحقق بالسيلفي (selfie_done)
// L3: L2 + صورة + بيانات أساسية مكتملة (مدينة، مهنة، تعليم)
export async function trustLevel(userId) {
  const f = await fields(userId);
  if (f.selfie_done === '1') {
    if (f.photo_done === '1' && f.city && f.profession && f.education) return 3;
    return 2;
  }
  return 1;
}

export async function trustFactors(userId) {
  const f = await fields(userId);
  return [
    { key: 'phone', label: 'رقم موثّق', done: true },
    { key: 'selfie', label: 'تحقق بالسيلفي', done: f.selfie_done === '1' },
    { key: 'photo', label: 'صورة شخصية', done: f.photo_done === '1' },
    { key: 'basics', label: 'بيانات أساسية', done: !!(f.city && f.profession && f.education) },
  ];
}
