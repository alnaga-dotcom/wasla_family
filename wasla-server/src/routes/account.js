...
router.get('/me/account/status', authGrace, ah(async (req, res) => {
  const u = await db.prepare('SELECT deleted_at FROM users WHERE id = ?').get(req.userId);
...
router.delete('/me/account', authRequired, ah(async (req, res) => {
  const reason = (req.body && req.body.reason ? String(req.body.reason) : '').trim();
  const deletedAt = new Date(Date.now() + deleteGraceMs()).toISOString().replace('T', ' ').slice(0, 19);
  await db.prepare('UPDATE users SET deleted_at = ? WHERE id = ?').run(deletedAt, req.userId);
  await db.prepare('DELETE FROM sessions WHERE user_id = ?').run(req.userId);
  if (reason) await db.prepare('UPDATE users SET deleted_at = deleted_at WHERE id = ?').run(req.userId); // no-op guard
  res.json({
    deleted: true,
...
router.post('/me/account/restore', authGrace, ah(async (req, res) => {
  const u = await db.prepare('SELECT deleted_at FROM users WHERE id = ?').get(req.userId);
  ...
  await db.prepare('UPDATE users SET deleted_at = NULL WHERE id = ?').run(req.userId);
...
router.get('/me/data', authGrace, ah(async (req, res) => {
