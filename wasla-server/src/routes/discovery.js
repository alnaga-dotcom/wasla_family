...
router.get('/recommendations', authRequired, emailVerifiedRequired, ah(async (req, res) => {
  const me = req.userId;
  const limit = Math.min(50, Math.max(1, Number(req.query.limit) || 20));
  const ranked = await getRecommendations(me, limit);

  const recommendations = (await Promise.all(ranked.map(async (r) => {
    const card = await publicCard(r.userId);
    if (!card) return null;
...
router.post('/recommendations/seen', authRequired, ah(async (req, res) => {
  const { userId } = req.body || {};
  const target = Number(userId);
  if (!Number.isInteger(target) || target === req.userId) {
    return apiError(res, 422, 'INVALID_USER', 'معرّف مستخدم غير صالح', 'userId');
  }
  await db.prepare('INSERT IGNORE INTO discovery_views (actor_id, target_id, created_at) VALUES (?, ?, ?)')
    .run(req.userId, target, nowIso());
  await markOpened(req.userId, target);
  res.json({ ok: true });
}));
