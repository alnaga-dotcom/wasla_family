...
router.post('/verification/request', authRequired, (req, res) => {
  const { type, note } = req.body || {};
  if (type && !['id', 'selfie'].includes(type)) {
    return apiError(res, 422, 'INVALID_TYPE', 'نوع التوثيق غير صالح', 'type');
  }
  const result = requestVerification(req.userId, type || 'id', String(note || '').slice(0, 500) || null);
  if (result.duplicate) {
...
router.get('/verification/me', authRequired, (req, res) => {
  res.json(myVerificationStatus(req.userId));
});

// GET /api/admin/verification?status=pending|all
router.get('/admin/verification', permissionRequired('verification', 'review'), (req, res) => {
  res.json({ requests: listVerificationRequests(req.query.status || 'pending') });
});

// POST /api/admin/verification/{id}/decision  { approve: bool, reason? }
router.post('/admin/verification/:id/decision', permissionRequired('verification', 'approve'), async (req, res) => {
  const id = Number(req.params.id);
  const approve = !!(req.body && req.body.approve);
  const result = decideVerification(id, approve, req.userId, String((req.body && req.body.reason) || '').slice(0, 500) || null);
