...
router.get('/notifications', authRequired, ah(async (req, res) => {
  ...
  res.json({ notifications: rows, unread: await unreadCount(req.userId) });
}));

// POST /api/notifications/read  { id? } — id اختياريًا؛ بدونه يُعلَّم الكل كمقروءة
router.post('/notifications/read', authRequired, ah(async (req, res) => {
  const id = Number((req.body && req.body.id) || 0);
  if (Number.isInteger(id) && id > 0) {
    const r = await db.prepare('UPDATE notifications SET is_read = 1 WHERE user_id = ? AND id = ?')
      .run(req.userId, id);
  ...
  res.json({ ok: true, unread: await unreadCount(req.userId) });
}));
