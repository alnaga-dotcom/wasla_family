...
export async function profileFields(userId) {
  const map = {};
  (await db.prepare('SELECT field_key, value FROM profile_fields WHERE user_id = ?')
    .all(userId))
...
export async function publicCard(userId) {
  ...
  const avatar = await db.prepare("SELECT filename FROM user_photos WHERE user_id = ? AND kind = 'profile' AND status = 'active' AND review_status = 'approved' ORDER BY id DESC LIMIT 1").get(userId);
  card.photo = avatar ? `/api/photos/${avatar.filename}` : null;
  card.hasPhoto = fields.photo_done === '1';
  card.isVerified = !!u.verified_at;
  card.trustLevel = await trustLevel(userId);
  return card;
}
