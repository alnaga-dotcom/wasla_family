...
const DEFAULT_APP_URL = process.env.WASLA_APP_URL || 'http://127.0.0.1:8081';
...
const server = createServer((req, res) => {
  ...
  if (url.pathname === '/config.json') {
    ...
    const data = existsSync(cfgPath)
      ...
      : Buffer.from(JSON.stringify({ apiBase: DEFAULT_API_BASE, appUrl: DEFAULT_APP_URL }, null, 2));
...
server.listen(PORT, () => {
  console.log(`Wasla landing served at http://127.0.0.1:${PORT}`);
});
